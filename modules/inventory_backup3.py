...
class InventoryModule(QWidget):
    ...
    def set_form_enabled(self, enabled: bool):
        ...
        # Update the style to indicate enabled/disabled state
        style = """
            QLineEdit, QSpinBox, QDoubleSpinBox {
                padding: 8px;
                border: 2px solid #bdc3c7;
        ...
        """ % ("white" if enabled else "#ecf0f1")
        
        self.name_input.setStyleSheet(style)
        self.desc_input.setStyleSheet(style)
        self.price_input.setStyleSheet(style)
        self.qty_input.setStyleSheet(style)
        self.min_qty_input.setStyleSheet(style)
    ...
    def load_products(self):
        try:
            with DatabaseConnection() as conn:
                ...
                products = cursor.fetchall()                self.products_table.setRowCount(len(products))
