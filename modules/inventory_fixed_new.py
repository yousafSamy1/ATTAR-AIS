...
class InventoryModule(QWidget):
    ...
    def init_ui(self):
        ...
        self.buttons_container.addWidget(self.add_btn)
        self.form_layout.addLayout(self.buttons_container)
    ...
    def edit_selected_product(self):
        ...
        try:
            ...
            if not hasattr(self, 'update_btn') or self.update_btn is None:
                ...
                self.buttons_container.addWidget(self.update_btn)
            ...
            with DatabaseConnection() as conn:
                ...
                for i, product in enumerate(products):
                    for j, value in enumerate(product):
                        ...
                        item.setBackground(QColor("#2c3e50"))  # Modern dark background
                        item.setForeground(QColor("white"))  # White text
                ...
                for row in range(self.products_table.rowCount()):
                    ...
                    if item_qty and item_min_qty:
                        try:
                            quantity = int(item_qty.text())
                            min_quantity = int(item_min_qty.text())
                            
                            if quantity <= min_quantity:
                                for col in range(self.products_table.columnCount()):
                                    item = self.products_table.item(row, col)
                                    if item:
                                        item.setBackground(QColor("#c0392b"))  # Modern red for low stock
                ...
                for i in range(self.products_table.columnCount()):
                    self.products_table.horizontalHeader().setSectionResizeMode(i, QHeaderView.ResizeMode.ResizeToContents)
